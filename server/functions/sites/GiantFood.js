    ...
    async getRelevantPrice(itemBodies, itemName) {
        ...
        try {
            for (var x = 0; itemBodies.length; x++) {
                // Get price endpoint
                const response = await this.client.get(`https://www.instacart.com/graphql?operationName=ItemPricesQuery&variables={"ids":["${itemBodies[x]['id']}"],"shopId":"4192","zoneId":"37","postalCode":"20902"}&extensions={"persistedQuery":{"version":1,"sha256Hash":"6bc7919897d4104897f991ab9e6f544aa2157e60781606871bf236f30e50243f"}}`, {
                    headers: {
                        "accept-encoding": 'none',
                        'sec-ch-device-memory': '8',
                        'sec-ch-ua': '"Google Chrome";v="107", "Chromium";v="107", "Not=A?Brand";v="24"',
                ...
                itemResult.push({
                    ...
                    'query': itemName,
                    'link': `https://www.instacart.com/store/giant/products/${itemBodies[x]['id'].split('-')[1]}`
                })
            }
        } catch (err) {
            console.log(err);
