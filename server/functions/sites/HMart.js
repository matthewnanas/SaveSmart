...
class HMart {
    ...
    async getRelevant(item) {
        try {
            // Get item endpoint
            const response = await this.client.get(`https://www.instacart.com/graphql?operationName=SearchResultsPlacements&variables={"filters":[],"action":null,"query":"${item.replace(' ', '%20')}","pageViewId":"cac76ffa-efe2-5ad4-a5a5-3fc7d5b22a92","retailerInventorySessionToken":"v1.47ac7a5.3004281003-20853-03910x17709-1-119-2322-0","elevatedProductId":null,"searchSource":"search","disableReformulation":false,"orderBy":"default","clusterId":null,"includeDebugInfo":false,"clusteringStrategy":null,"contentManagementSearchParams":{"itemGridColumnCount":10,"aisleGridVericalCarouselCount":2,"aisleGridVericalCarouselItemGridVisibleColumnCount":2,"aisleGridVericalCarouselItemGridCount":1},"shopId":"10661"}&extensions={"persistedQuery":{"version":1,"sha256Hash":"19f3d8fe269c71ba489504c3a3eaccf617a39608dcda8e92bb3c35d46fe48eca"}}`, {
                headers: {
    ...
    async getRelevantPrice(itemBodies, itemName) {
        ...
        try {
            for (var x = 0; itemBodies.length; x++) {
                // Get price endpoint
                const response = await this.client.get(`https://www.instacart.com/graphql?operationName=ItemPricesQuery&variables={"ids":["${itemBodies[x]['id']}"],"shopId":"10661","zoneId":"211","postalCode":"20853"}&extensions={"persistedQuery":{"version":1,"sha256Hash":"d5b93a3b496b2c40b38af695ba3391eae314a084783ec42aedd8f1a459bd461d"}}`, {
                    headers: {
                        "accept-encoding": 'none',
                        'sec-ch-device-memory': '8',
                        'sec-ch-ua': '"Google Chrome";v="107", "Chromium";v="107", "Not=A?Brand";v="24"',
                ...
                itemResult.push({
                    ...
                    'query': itemName,
                    'link': `https://www.instacart.com/store/hmart/products/${itemBodies[x]['id'].split('-')[1]}`
                })
            }
        } catch (err) {
            console.log(err);
